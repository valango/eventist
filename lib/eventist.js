...
!(function (exports) {
  ...
  var fail = function (msg, method) {
    throw new TypeError(['Eventist#', method, ': ', msg].join(''));
  };

  /* Polyfill to avoid dependencies. */
  var callable = function (v, method) {
        if (FUNCTION !== typeof v) {
          fail('A callable argument is expected!', method);
        }
      }
    , string   = function (v, method) {
        if (STRING !== typeof v) {
          fail('Event type must be string!', method);
        }
      };
