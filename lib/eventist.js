...
!(function (exports) {
  ...
  proto.emit = function () {
    var handlers,
        res  = UNDEF, args, exec, event, i,
        rep = this._reporter, count = 0,
        hook = this._hook;

    args = slice.call(arguments);
    hook && hook.call(this, args);
    event = args.shift(1);
      ...
      try {
        ...
        if (!callback) {
          if (!e.hasOwnProperty('message')) {
            // Chack for some specual cases like Network error and alike.
            e = new Error(e + '');
          }
          e.message = 'Sending "' + args[0] + '": ' + e.message;
          throw e;
        }
        err = e;
      }
